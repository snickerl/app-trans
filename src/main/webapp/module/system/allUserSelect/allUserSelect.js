function onsqPageLoaded(){
	...
	var grid = LUI.Grid.getInstance('allUserSelectGrid');
...
function searchBy(){
	var filters = [];
	var selectedNode = LUI.Tree.getInstance('allUserSelectTree').getSelectNode();
	...
	LUI.Datasource.getInstance('allUserSelectDatasource').load({
      filters:filters
    });
}